...
async def get_spec_street_by_id(
    ...
    async with new_session() as session:
        spec_street = await spec_street_data.get_by_id(
            session, 
            spec_street_id, 
...
async def get_spec_streets_paginated(
    ...
    async with new_session() as session:
        items, total = await spec_street_data.get_paginated(
            session=session,
...
async def get_all_spec_streets(
    ...
    async with new_session() as session:
        return await spec_street_data.get_all(session, load_streets=load_streets)
...
async def get_spec_street_options(
    ...
    async with new_session() as session:
        return await spec_street_data.get_options(session)
...
async def get_spec_street_with_stats(
    ...
    async with new_session() as session:
        ...
        streets_count = await spec_street_data.count_streets(
            session, 
            spec_street_id
        )
...
async def create_spec_street(
    ...
    async with new_session() as session:
        ...
        if await spec_street_data.check_name_exists(session, spec_street_create.name):
            raise HTTPException(
                status_code=400,
        ...
        # Создание
        spec_street = await spec_street_data.create(session, spec_street_create)
...
async def update_spec_street(
    ...
    async with new_session() as session:
        ...
        existing = await spec_street_data.get_by_id(session, spec_street_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Тип улицы с id {spec_street_id} не найден"
        ...
        if spec_street_update.name and spec_street_update.name != existing.name:
            if await spec_street_data.check_name_exists(session, spec_street_update.name, spec_street_id):
                raise HTTPException(
                    status_code=400,
        ...
        # Обновление
        spec_street = await spec_street_data.update(session, spec_street_id, spec_street_update)
...
async def delete_spec_street(
    ...
    async with new_session() as session:
        ...
        if spec_street.streets and len(spec_street.streets) > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Невозможно удалить тип улицы '{spec_street.name}': есть связанные улицы ({len(spec_street.streets)} шт.)"
            )
        
        # Удаление
        success = await spec_street_data.delete(session, spec_street_id)
