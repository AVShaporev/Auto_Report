...
async def get_locality_by_id(
    ...
    async with new_session() as session:
        locality = await locality_data.get_by_id(
            session, 
            locality_id, 
...
async def get_localities_paginated(
    ...
    async with new_session() as session:
        items, total = await locality_data.get_paginated(
            session=session,
...
async def get_all_localities(
    ...
    async with new_session() as session:
        return await locality_data.get_all(session, load_relations=load_relations)
...
async def get_locality_options(
    ...
    async with new_session() as session:
        return await locality_data.get_options(session)
...
async def get_locality_options_by_spec(
    ...
    async with new_session() as session:
        ...
        if not await spec_locality_data.check_name_exists(session, spec_locality_id):
            raise HTTPException(
                status_code=404,
                detail=f"Тип населенного пункта с id {spec_locality_id} не найден"
            )
        
        return await locality_data.get_options_by_spec_locality(session, spec_locality_id)
...
async def get_locality_with_stats(
    ...
    async with new_session() as session:
        ...
        organizations_count = await locality_data.count_organizations(session, locality_id)
        objects_count = await locality_data.count_objects(session, locality_id)
...
async def get_localities_paginated_with_stats(
    ...
    async with new_session() as session:
        ...
        result_items = []
        for item in items:
            organizations_count = await locality_data.count_organizations(session, item.id)
            objects_count = await locality_data.count_objects(session, item.id)
...
async def create_locality(
    ...
    async with new_session() as session:
        ...
        if await locality_data.check_name_exists(session, locality_create.name):
            raise HTTPException(
                status_code=400,
        ...
        if locality_create.spec_locality_id:
            if not await locality_data.check_spec_locality_exists(session, locality_create.spec_locality_id):
                raise HTTPException(
                    status_code=400,
                    detail=f"Тип населенного пункта с id {locality_create.spec_locality_id} не существует"
                )
        
        # Создание
        locality = await locality_data.create(session, locality_create)
...
async def update_locality(
    ...
    async with new_session() as session:
        ...
        existing = await locality_data.get_by_id(session, locality_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Населенный пункт с id {locality_id} не найден"
        ...
        if locality_update.name and locality_update.name != existing.name:
            if await locality_data.check_name_exists(session, locality_update.name, locality_id):
                raise HTTPException(
                    status_code=400,
                    detail=f"Населенный пункт с названием '{locality_update.name}' уже существует"
                )
        ...
        if locality_update.spec_locality_id and locality_update.spec_locality_id != existing.spec_locality_id:
            if not await locality_data.check_spec_locality_exists(session, locality_update.spec_locality_id):
                raise HTTPException(
                    status_code=400,
                    detail=f"Тип населенного пункта с id {locality_update.spec_locality_id} не существует"
                )
        
        # Обновление
        locality = await locality_data.update(session, locality_id, locality_update)
...
async def delete_locality(
    ...
    async with new_session() as session:
        ...
        if locality.organizations and len(locality.organizations) > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Невозможно удалить населенный пункт '{locality.name}': есть связанные организации ({len(locality.organizations)} шт.)"
            )
        ...
        if locality.objects and len(locality.objects) > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Невозможно удалить населенный пункт '{locality.name}': есть связанные объекты ({len(locality.objects)} шт.)"
            )
        
        # Удаление
        success = await locality_data.delete(session, locality_id)
