...
async def get_street_by_id(
    ...
    async with new_session() as session:
        street = await street_data.get_by_id(
            session, 
            street_id, 
...
async def get_streets_paginated(
    ...
    async with new_session() as session:
        items, total = await street_data.get_paginated(
            session=session,
...
async def get_all_streets(
    ...
    async with new_session() as session:
        return await street_data.get_all(session, load_relations=load_relations)
...
async def get_street_options(
    ...
    async with new_session() as session:
        return await street_data.get_options(session)
...
async def get_street_options_by_spec(
    ...
    async with new_session() as session:
        ...
        if not await spec_street_data.check_name_exists(session, spec_street_id):
            raise HTTPException(
                status_code=404,
                detail=f"Тип улицы с id {spec_street_id} не найден"
            )
        
        return await street_data.get_options_by_spec_street(session, spec_street_id)
...
async def get_street_with_stats(
    ...
    async with new_session() as session:
        ...
        organizations_count = await street_data.count_organizations(session, street_id)
        objects_count = await street_data.count_objects(session, street_id)
...
async def get_streets_paginated_with_stats(
    ...
    async with new_session() as session:
        ...
        result_items = []
        for item in items:
            organizations_count = await street_data.count_organizations(session, item.id)
            objects_count = await street_data.count_objects(session, item.id)
...
async def create_street(
    ...
    async with new_session() as session:
        ...
        if await street_data.check_name_exists(session, street_create.name):
            raise HTTPException(
                status_code=400,
        ...
        if not await street_data.check_spec_street_exists(session, street_create.spec_street_id):
            raise HTTPException(
                status_code=400,
        ...
        # Создание
        street = await street_data.create(session, street_create)
...
async def update_street(
    ...
    async with new_session() as session:
        ...
        existing = await street_data.get_by_id(session, street_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Улица с id {street_id} не найдена"
        ...
        if street_update.name and street_update.name != existing.name:
            if await street_data.check_name_exists(session, street_update.name, street_id):
                raise HTTPException(
                    status_code=400,
        ...
        if street_update.spec_street_id and street_update.spec_street_id != existing.spec_street_id:
            if not await street_data.check_spec_street_exists(session, street_update.spec_street_id):
                raise HTTPException(
                    status_code=400,
        ...
        # Обновление
        street = await street_data.update(session, street_id, street_update)
...
async def delete_street(
    ...
    async with new_session() as session:
        ...
        # Удаление
        success = await street_data.delete(session, street_id)
