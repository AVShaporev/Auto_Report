...
async def get_regions_paginated(
    ...
    async with new_session() as session:
        items, total = await region_data.get_paginated(
            session=session,
...
async def get_all_regions(
    ...
    async with new_session() as session:
        return await region_data.get_all(session, load_relations=load_relations)
...
async def get_region_options(
    ...
    async with new_session() as session:
        return await region_data.get_options(session)
...
async def get_region_with_stats(
    ...
    async with new_session() as session:
        ...
        organizations_count = await region_data.count_organizations(session, region_id)
        objects_count = await region_data.count_objects(session, region_id)
...
async def get_regions_paginated_with_stats(
    ...
    async with new_session() as session:
        ...
        result_items = []
        for item in items:
            organizations_count = await region_data.count_organizations(session, item.id)
            objects_count = await region_data.count_objects(session, item.id)
...
async def create_region(
    ...
    async with new_session() as session:
        ...
        if await region_data.check_name_exists(session, region_create.name):
            raise HTTPException(
                status_code=400,
        ...
        if await region_data.check_symbol_exists(session, region_create.symbol):
            raise HTTPException(
                status_code=400,
        ...
        if not await region_data.check_spec_region_exists(session, region_create.spec_region_id):
            raise HTTPException(
                status_code=400,
        ...
        # Создание
        region = await region_data.create(session, region_create)
...
async def update_region(
    ...
    async with new_session() as session:
        ...
        existing = await region_data.get_by_id(session, region_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Регион с id {region_id} не найден"
        ...
        if region_update.name and region_update.name != existing.name:
            if await region_data.check_name_exists(session, region_update.name, region_id):
                raise HTTPException(
                    status_code=400,
        ...
        if region_update.symbol and region_update.symbol != existing.symbol:
            if await region_data.check_symbol_exists(session, region_update.symbol, region_id):
                raise HTTPException(
                    status_code=400,
        ...
        if region_update.spec_region_id and region_update.spec_region_id != existing.spec_region_id:
            if not await region_data.check_spec_region_exists(session, region_update.spec_region_id):
                raise HTTPException(
                    status_code=400,
        ...
        # Обновление
        region = await region_data.update(session, region_id, region_update)
...
async def delete_region(
    ...
    async with new_session() as session:
        ...
        if region.organizations and len(region.organizations) > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Невозможно удалить регион '{region.name}': есть связанные организации ({len(region.organizations)} шт.)"
            )
        ...
        if region.objects and len(region.objects) > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Невозможно удалить регион '{region.name}': есть связанные объекты ({len(region.objects)} шт.)"
            )
        
        # Удаление
        success = await region_data.delete(session, region_id)
