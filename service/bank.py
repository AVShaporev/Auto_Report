...
async def get_bank_by_id(
    ...
    async with new_session() as session:
        bank = await bank_data.get_bank_by_id(
                                                session, 
                                                bank_id, 
...
async def get_banks_paginated(
    ...
    async with new_session() as session:
        items, total = await bank_data.get_bank_paginated(
                                                        session=session,
...
async def get_all_banks(
    ...
    async with new_session() as session:
        return await bank_data.get_bank_all(
                                            session, 
                                            load_organizations=load_organizations
                                            )
...
async def get_bank_with_stats(
    ...
    async with new_session() as session:
        ...
        organizations_count = await bank_data.count_bank_organizations(
                                                                        session, 
                                                                        bank_id
                                                                        )
...
async def create_bank(
    ...
    async with new_session() as session:
        ...
        if await bank_data.check_bank_name_exists(session, bank_create.name):
            raise HTTPException(
                                status_code=400,
        ...
        if await bank_data.check_bank_bik_exists(session, bank_create.bik):
            raise HTTPException(
                                status_code=400,
        ...
        if await bank_data.check_bank_inn_exists(session, bank_create.inn):
            raise HTTPException(
                                status_code=400,
        ...
        # Создание
        bank = await bank_data.create_bank(session, bank_create)
...
async def update_bank(
    ...
    async with new_session() as session:
        ...
        existing = await bank_data.get_bank_by_id(session, bank_id)
        if not existing:
            raise HTTPException(
                                status_code=404,
                                detail=f"Банк с id {bank_id} не найден"
        ...
        if bank_update.name and bank_update.name != existing.name:
            if await bank_data.check_bank_name_exists(session, bank_update.name, bank_id):
                raise HTTPException(
                                    status_code=400,
        ...
        if bank_update.bik and bank_update.bik != existing.bik:
            if await bank_data.check_bank_bik_exists(session, bank_update.bik, bank_id):
                raise HTTPException(
                                    status_code=400,
        ...
        if bank_update.inn and bank_update.inn != existing.inn:
            if await bank_data.check_bank_inn_exists(session, bank_update.inn, bank_id):
                raise HTTPException(
                                    status_code=400,
        ...
        # Обновление
        bank = await bank_data.update_bank(session, bank_id, bank_update)
...
async def delete_bank(
    ...
    async with new_session() as session:
        ...
        # Удаление
        success = await bank_data.delete_bank(session, bank_id)
