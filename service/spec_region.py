...
async def get_spec_region_by_id(
    ...
    async with new_session() as session:
        spec_region = await spec_region_data.get_by_id(
            session, 
            spec_region_id, 
...
async def get_spec_regions_paginated(
    ...
    async with new_session() as session:
        items, total = await spec_region_data.get_paginated(
            session=session,
...
async def get_all_spec_regions(
    ...
    async with new_session() as session:
        return await spec_region_data.get_all(session, load_regions=load_regions)
...
async def get_spec_region_options(
    ...
    async with new_session() as session:
        return await spec_region_data.get_options(session)
...
async def get_spec_region_with_stats(
    ...
    async with new_session() as session:
        ...
        regions_count = await spec_region_data.count_regions(
            session, 
            spec_region_id
        )
...
async def create_spec_region(
    ...
    async with new_session() as session:
        ...
        if await spec_region_data.check_name_exists(session, spec_region_create.name):
            raise HTTPException(
                status_code=400,
        ...
        # Создание
        spec_region = await spec_region_data.create(session, spec_region_create)
...
async def update_spec_region(
    ...
    async with new_session() as session:
        ...
        existing = await spec_region_data.get_by_id(session, spec_region_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Тип региона с id {spec_region_id} не найден"
        ...
        if spec_region_update.name and spec_region_update.name != existing.name:
            if await spec_region_data.check_name_exists(session, spec_region_update.name, spec_region_id):
                raise HTTPException(
                    status_code=400,
        ...
        # Обновление
        spec_region = await spec_region_data.update(session, spec_region_id, spec_region_update)
...
async def delete_spec_region(
    ...
    async with new_session() as session:
        ...
        # Удаление
        success = await spec_region_data.delete(session, spec_region_id)
