...
async def get_organization_by_id(
    ...
    async with new_session() as session:
        organization = await org_data.get_organization_by_id(
                                                                session,
                                                                org_id,
                                                                load_relations
                                                                )
...
async def get_organizations_executor_paginated(
    ...
    async with new_session() as session:
        organizations, total = await org_data.get_organization_paginated(
                                                                        session=session,
...
async def create_organization(
    ...
    async with new_session() as session:
        ...
        if await org_data.check_organization_name_exists(session, org_create.name):
            raise HTTPException(
                                status_code=400,
        ...
        if await org_data.check_organization_short_name_exists(session, org_create.short_name):
            raise HTTPException(
                                status_code=400,
        ...
        if await org_data.check_organization_inn_exists(session, org_create.inn):
            raise HTTPException(
                                status_code=400,
        ...
        organization = await org_data.create_organization(session, org_create)
...
async def update_organization(
    ...
    async with new_session() as session:
        ...
        existing = await org_data.get_organization_by_id(session, org_id, load_relations=False)
        if not existing:
            raise HTTPException(
                                status_code=404,
                                detail=f"Организация с id {org_id} не найдена"
        ...
        if org_update.name and org_update.name != existing.name:
            if await org_data.check_organization_name_exists(session, org_update.name, org_id):
                raise HTTPException(
                                    status_code=400,
        ...
        if org_update.short_name and org_update.short_name != existing.short_name:
            if await org_data.check_organization_short_name_exists(
                                                                    session,
                                                                    org_update.short_name,
                                                                    org_id):
                raise HTTPException(
                                    status_code=400,
                                    detail=f"Организация с сокращенным названием \
                                    '{org_update.short_name}' уже существует"
                                    )
        
        # Проверка уникальности ИНН
        if org_update.inn and org_update.inn != existing.inn:
            if await org_data.check_organization_inn_exists(
                                                            session,
                                                            org_update.inn,
                                                            org_id
                                                            ):
                raise HTTPException(
                                        status_code=400,
                                        detail=f"Организация с ИНН \
                                        '{org_update.inn}' уже существует"
                                    )
        
        # Обновление
        organization = await org_data.update_organization(
                                                            session,
                                                            org_id,
                                                            org_update
                                                            )
