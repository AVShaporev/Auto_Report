...
router = APIRouter(prefix='/api/contract', tags=['API'])

@router.get('/list')
async def get_contracts_api(request: Request, user: User = Depends(get_current_user)):
...
@router.post('/create')
async def post_create_contract(number,
                                        description,
                                        user: User = Depends(get_current_user)):
