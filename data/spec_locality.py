...
@timer
async def get_spec_locality_by_name(
    session: AsyncSession,
    name: str
) -> Optional[Spec_Locality]:
...
@timer
async def get_spec_locality_all(
    session: AsyncSession,
    *,
    load_localities: bool = False
) -> List[Spec_Locality]:
...
@timer
async def get_spec_locality_paginated(
    session: AsyncSession,
    skip: int = 0,
    limit: int = 20,
...
@timer
async def check_spec_locality_name_exists(
    session: AsyncSession,
    name: str,
...
@timer
async def count_localities_by_spec_locality(
    session: AsyncSession,
    spec_locality_id: int
) -> int:
    """
...
async def create_spec_locality(
    ...
    spec_locality = Spec_Locality(**spec_locality_create.model_dump())
...
async def update_spec_locality(
    ...
    spec_locality = await get_spec_locality_by_id(session, spec_locality_id, load_localities=False)
    if not spec_locality:
        return None

    update_data = spec_locality_update.model_dump(exclude_unset=True)
