...
@timer
async def get_spec_order_by_name(
    session: AsyncSession,
    name: str
) -> Optional[Spec_Order]:
...
@timer
async def get_spec_order_paginated(
    session: AsyncSession,
    skip: int = 0,
    limit: int = 20,
...
@timer
async def check_spec_order_name_exists(
    session: AsyncSession,
    name: str,
...
@timer
async def count_orders_by_spec_order(
    session: AsyncSession,
    spec_order_id: int
) -> int:
    """
...
async def create_spec_order(
    ...
    spec_order = Spec_Order(**spec_order_create.model_dump())
...
async def update_spec_order(
    ...
    """Обновить тип заявки"""
    spec_order = await get_spec_order_by_id(session, spec_order_id, load_orders=False)
    if not spec_order:
        return None

    update_data = spec_order_update.model_dump(exclude_unset=True)
