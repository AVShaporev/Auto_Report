...
def timer(func):
    ...
    Выводит в консоль время старта, окончания и затраченное время.
    """
    if asyncio.iscoroutinefunction(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            print(f"[{start_str}] Начало выполнения {func.__name__}")
            ...
            elapsed = end - start
            print(f"[{end_str}] Конец выполнения {func.__name__} (затрачено: {elapsed:.4f} с)")
            return result
