...
### Added
- `OrderResponse.object_requires_signature` — мобилка ещё в форме отчёта
  (в т. ч. офлайн, из кэша заявки) предупреждает, что подпись представителя
  заказчика на объекте обязательна.

## [1.0.71] — 2026-09-19

### Added
- `ReportResponse.object_requires_signature` — на объекте отчёта включено
  «подтверждение подписью ответственного»: веб и мобилка показывают, что
  подпись обязательна, до попытки отправить на утверждение.
